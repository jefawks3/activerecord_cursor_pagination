...
RSpec.describe ActiverecordCursorPagination::CursorScope do
  ...
  subject do
    described_class.new Post, default_query, nil, per: 10
  end

  let :empty_subject do
    described_class.new Post, empty_query, nil, per: 10
  end

  let :single_subject do
    described_class.new Post, single_query, nil, per: 10
  end

  let :single_record_scoped_subject do
    described_class.new Post, default_query, Post.find(80), per: 1
  end

  let :first_page_subject do
    described_class.new Post, default_query, "1LApYQ0cIJawthNSn6OXbmEFQwb7SS99WBsgBSFGqE7gV4099l5a7qkuLkuDnZBJC9ckN61klw1X/OyGMnFcrYhugAcmWdWJ5cxEO8mzIfktm+lGRmJDks+yzJD8hybP", per: 10
  end

  let :second_page_subject do
    described_class.new Post, default_query, "6hT2IhaiLHq+4AW9Wp9fr6PesPUPnPts9T7H9eThRzwsB6w8sDdvsKyTtCJYhyiZrKH0uvtPwv3DFe4v+7AonkVRzF8s73IJQC55C18SOCupPUfz5pCR25RSnAW/1hVf", per: 10
  end
  ...
  let :last_page_subject do
    ...
    described_class.new Post, default_query, cursor, per: 10
  end

  let :record_subject do
    described_class.new Post, default_query, Post.find(50), per: 10
  end
  ...
  describe ".initialize" do
    context "invalid cursor" do
      ...
      it "raises InvalidCursorError" do
        expect do
          described_class.new Post, default_query, invalid_cursor, per: 20
    ...
    context "invalid cursor type" do
      it "raises InvalidCursorError" do
        expect do
          described_class.new Post, default_query, 1, per: 20
        end.to raise_error(ActiverecordCursorPagination::InvalidCursorError)
      end
    end
  end
  ...
  describe ".single_record?" do
    context "when per page = 1" do
      subject { described_class.new Post, default_query, nil, per: 1 }
    ...
    context "when per page > 1" do
      subject { described_class.new Post, default_query, nil, per: 10 }
  ...
  describe ".each_with_index" do
    it "executes for each record" do
      count = 0
      subject.each_with_index { count += 1 }
      expect(count).to be 10
    end
  ...
  describe ".map_with_index" do
    it "executes for each record" do
      count = 0
      subject.map_with_index { count += 1 }
      expect(count).to be 10
    end
