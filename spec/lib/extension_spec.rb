RSpec.describe ActiverecordCursorPagination::Extension do
  ...
  context '.cursor_batch' do
    it 'iterates through each page' do
      count = 0
      Post.cursor_batch(batch_size: 10) { |cursor| count += 1 }
  ...
  context '.cursor_batch_with_index' do
    it 'iterates through each page' do
      count = 0
      Post.cursor_batch_with_index(batch_size: 10) { |cursor, index| count += 1 }
      expect(count).to eq 2
    end
  ...
  context '.cursor_find_each' do
    it 'iterates through each record' do
      count = 0
      Post.cursor_find_each(batch_size: 5) { |post| count += 1 }
  ...
  context '.cursor_find_each_with_index' do
    it 'iterates through each record' do
      count = 0
      Post.cursor_find_each_with_index(batch_size: 5) { |post, index| count += 1 }
      expect(count).to eq 20
    end
